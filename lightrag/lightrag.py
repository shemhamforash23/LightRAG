...
class LightRAG:
    ...
    def __post_init__(self):
        ...
        assert (
            self.embedding_func is not None
        ), "Embedding function must be set before use"
    ...
    async def adelete_by_doc_id(self, doc_id: str) -> None:
        ...
        try:
            ...
            for chunk_id in chunk_ids:
                ...
                # Check relationships
                relations = await self.relationships_vdb.get_relations_by_source_id(
                    chunk_id
                )
                    ...
                    for item in data_with_chunk:
                        ...
                        if not chunk_ids_list:
                            ...
                            if data_type == "entities":
                                ...
                                if not data_for_vdb[item_id].get("content"):
                                    src_id = item.get("source_id", "") or item.get(
                                        "id", ""
                                    )
            ...
            async def verify_deletion():
                ...
                remaining_related_chunks = await self.text_chunks.get_chunks_by_doc_id(
                    doc_id
                )
    ...
    async def aexport_data(
        ...
        for chunk_id in all_chunks.keys():
            chunk_relations = await self.relationships_vdb.get_relations_by_source_id(
                chunk_id
            )
            ...
            with pd.ExcelWriter(output_path, engine="xlsxwriter") as excel_writer:
                if not entities_df.empty:
                    entities_df.to_excel(
                        excel_writer, sheet_name="Entities", index=False
                    )
                if not relations_df.empty:
                    relations_df.to_excel(
                        excel_writer, sheet_name="Relations", index=False
                    )
                if not relationships_df.empty:
                    relationships_df.to_excel(
                        excel_writer, sheet_name="Relationships", index=False
                    )
